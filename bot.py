...
def system_menu(lang: str = "ua", is_admin: bool = False, is_tutor: bool = False):
    ...
    tutor_button = TEXTS[lang]["tutor_profile_btn"] if is_tutor else TEXTS[lang]["tutor_login_btn"]
    admin_button = TEXTS[lang]["admin_profile_btn"] if is_admin else TEXTS[lang]["admin_login_btn"]
    kb.row(tutor_button, admin_button)

    extra_buttons = []
    if is_tutor:
        extra_buttons.append(TEXTS[lang]["tutor_logout_btn"])
    if is_admin:
        extra_buttons.append(TEXTS[lang]["admin_logout_btn"])

    if len(extra_buttons) == 2:
        kb.row(extra_buttons[0], extra_buttons[1])
    elif len(extra_buttons) == 1:
        kb.row(extra_buttons[0])

    kb.row(TEXTS[lang]["premium_menu_btn"], TEXTS[lang]["change_language_btn"])
    kb.row(TEXTS[lang]["back"])
    return kb
